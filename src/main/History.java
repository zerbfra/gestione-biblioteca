package main;

import java.awt.*;
import java.sql.*;

import javax.swing.*;
import javax.swing.table.*;





@SuppressWarnings("serial")


/** Mostra i libri noleggiati da un utente, una volta che ha fatto il login.
 *  Vengono visualizzati TUTTI i libri presi in prestito, tranne gli attuali.
 */
public class History extends JPanel {

	/** Costruttore del pannello, si occupa di inserire i vari elementi e formulare la query al database **/
	public History() {
	
		/* Inserisco un layout BorderLayout */
		this.setLayout(new BorderLayout(30,30));

		/* Titolo del pannello */
		JLabel title=new JLabel("Cronologia Prestiti",JLabel.CENTER);
		title.setFont(new Font("", Font.BOLD, 14));
		this.add(title,BorderLayout.NORTH);

		/* Pannello senza layout */
		JPanel grid = new JPanel();
		grid.setLayout(new FlowLayout());

		/* Inoltro la query, dopo aver reperito il nome utente dal login */		
		ResultSet rs = Database.Query("SELECT * from lending_view v,book b WHERE v.id_user = '"+LoginPanel.getUser().getId()+"' AND b.id = v.id_book AND v.returndate != '0000-00-00' ORDER BY v.returndate", Database.getConn(),false);
		
		/* Modello per la JTable, a cui vengono aggiunte le varie colonne */
		DefaultTableModel model = new DefaultTableModel();
		model.addColumn("Titolo");
		model.addColumn("Autore");
		model.addColumn("Editore");
		model.addColumn("Restituito il");
		
		
		try {
			while (rs.next()) {	
				/* Inserisco le righe con le informazioni */
				model.insertRow(model.getRowCount(), new Object[]{ rs.getString("title"), rs.getString("author"), rs.getString("publisher"), rs.getString("returndate") });

			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}

		/* Includo uno scroller nella tabella, in modo da visualizare tutti i record in una piccola schermata */
		JScrollPane scrollPane = new JScrollPane();
		/* Inizializzo la tabella e le assegno il modello creato e le relative righe */
		JTable table;
		table = new JTable(model);

		/* Gestisco il layout della tabella, in modo che sia graficamente comprensibile */
		table.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
		table.setEnabled(false); // non editabile
		table.getColumnModel().getColumn(0).setPreferredWidth(330);
		table.getColumnModel().getColumn(1).setPreferredWidth(130);
		table.getColumnModel().getColumn(2).setPreferredWidth(180);
		table.getColumnModel().getColumn(3).setPreferredWidth(155);


		/* Aggiungo lo scroller alla tabella e ne assegno una dimensione di default */
		scrollPane.getViewport().add(table);
		scrollPane.setPreferredSize(new Dimension(800,400));

		/* Aggiungo al pannello lo scroller (e di conseguenza la tabella in esso contenuta */
		grid.add(scrollPane);
		/* Aggiungo grid al pannello centralmente */
		this.add(grid,BorderLayout.CENTER);

	}

}
